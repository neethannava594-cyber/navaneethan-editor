-- ============================================
-- CUSTOMER ENQUIRY SYSTEM - SUPABASE SETUP
-- ============================================
...
-- Policy 2: Admins can read all enquiries (SELECT)
CREATE POLICY "Admins can read all enquiries" 
ON enquiries FOR SELECT 
USING (true);

-- Policy 3: Admins can update enquiries (UPDATE)
...
-- Status values: 'new', 'contacted', 'completed'

-- ============================================
-- TROUBLESHOOTING
-- ============================================

-- If table doesn't appear in UI after running SQL:
-- 1. Refresh the Supabase dashboard (F5)
-- 2. Check in SQL Editor → Tables
-- 3. Make sure you're in the correct project/database

-- If you get "table already exists" error:
-- The table was already created. Skip and use existing one.

-- If RLS policies don't work:
-- Make sure to enable RLS for the table first (ALTER TABLE enquiries ENABLE ROW LEVEL SECURITY;)
